...
function walkAssessment(hourly, idx, now, nextSun, tempF, isDay, cloud, uv, horizon){
  ...
  if (nextSun && decision.label.includes("Walk now")) {
    ...
    if (minutesToSun > 0 && minutesToSun <= 75) {
      ...
      return { primary: decision.label, secondary, tertiary, cls: "walk-golden" };
    }
  }
  ...
  return { primary: decision.label, secondary, tertiary, cls: decision.cls };
}
...
async function refresh(){
  try {
    ...
    if (bring.length) {
      $("bringValue").textContent = bring.join(" + ");
